...
namespace Zt\Cognito\Providers;

use Zt\Cognito\AwsCognito;
use Zt\Cognito\AwsCognitoClient;
use Zt\Cognito\AwsCognitoManager;
use Zt\Cognito\Guards\CognitoSessionGuard;
use Zt\Cognito\Guards\CognitoTokenGuard;

use Zt\Cognito\Http\Parser\Parser;
use Zt\Cognito\Http\Parser\AuthHeaders;
...
class AwsCognitoServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->alias('zt.aws.cognito', AwsCognito::class);
    }
    ...
    {
        //Request Parser
        $this->app->singleton('zt.aws.cognito.parser', function (Application $app) {
            $parser = new Parser(
                $app['request'],
                [
                    new AuthHeaders,
        ...
        //Storage Provider
        $this->app->singleton('zt.aws.cognito.provider.storage', function (Application $app) {
        ...
        //Aws Cognito Manager
        $this->app->singleton('zt.aws.cognito.manager', function (Application $app) {
            return (new AwsCognitoManager(
                $app['zt.aws.cognito.provider.storage']
            ));
        });

        $this->app->singleton('zt.aws.cognito', function (Application $app, array $config) {
            return (new AwsCognito(
                $app['zt.aws.cognito.manager'],
                $app['zt.aws.cognito.parser']
            ));
        });
    } //Function ends
